...
lnames = load(file = "/home/evelyn/Dropbox/project/bleww_final/Zoox_expr/fightzone/C/wgcna/test2/anothertry/wgcna3b_manual_sft10_minModSize30_cutDpth0.4_signed.Rdata") #C outliers removed
...
datTraits = datTraits %>%
  select(c_t,minorLR, logCDratio, logCcount)
datTraits$c = ifelse(datTraits$c_t == "c", 1,0)
datTraits$t = ifelse(datTraits$c_t== "h",1,0)
datTraits = datTraits %>%
  select(minorLR, logCDratio)
...
coul <- coul <- rev(colorRampPalette(brewer.pal(8, "RdYlBu"))(25))
pheatmap(t(MEsm),scale="row",cluster_cols=FALSE, labels_col = "",color = blueWhiteRed(50))
...
ggplot(data=datTraits1, aes(x=sample, y=logCDratio, group=1))+
...
ggplot(data=datTraits1, aes(x=sample,group=1))+
  geom_jitter(aes(y=minorLR),color="red")+
  geom_jitter(aes(y=logCDratio),color = "black")+
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
